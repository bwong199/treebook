...
class UserFriendshipTest < ActiveSupport::TestCase
	...
	context "#mutual_friendship" do 
		setup do 
			UserFriendship.request users(:jason), users(:ben)
			@friendship1 = users(:jason).user_friendships.where(friend_id: users(:ben).id).first
			@friendship2 = users(:ben).user_friendships.where(friend_id: users(:jason).id).first
		end
	...
	context "#accept_mutual_friendship!" do 
		...
		should "accept the mutual friendship" do 
			...
			friendship2.reload
			assert_equal "accepted", friend2.state
		end 
	end 
